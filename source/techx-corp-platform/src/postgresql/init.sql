...
CREATE UNIQUE INDEX users_email_unique ON identity.users (lower(email)) WHERE deleted_at IS NULL;
CREATE UNIQUE INDEX users_username_unique ON identity.users (lower(username)) WHERE deleted_at IS NULL;
...
CREATE INDEX sessions_user_id_index ON identity.sessions (user_id);
CREATE INDEX sessions_active_token_index ON identity.sessions (token_hash) WHERE revoked_at IS NULL;
...
CREATE UNIQUE INDEX product_review_user_unique ON reviews.productreviews (product_id, user_id)
...
-- Fill every product to 100 reviews for deterministic AI/non-functional testing.
WITH counts AS (
    SELECT p.id, count(r.id)::int AS current_count
    FROM catalog.products p LEFT JOIN reviews.productreviews r ON r.product_id = p.id AND r.deleted_at IS NULL GROUP BY p.id
)
INSERT INTO reviews.productreviews (product_id, username, description, score, created_at, updated_at)
SELECT c.id, 'seed_user_' || c.id || '_' || n,
       (ARRAY['Solid product and easy to use, but packaging could be better.', 'Good value for the price and reliable performance.',
       'Works as described; documentation needs more detail.', 'Excellent quality and a very good experience.',
       'Useful features, although setup took longer than expected.', 'Dependable and recommended for beginners.',
       'Acceptable quality, but advanced users may want more control.', 'Practical design and responsive support.'])[1 + ((n - 1) % 8)],
       (1 + ((n * 7 + length(c.id)) % 5))::numeric(2,1), now() - make_interval(days => (n % 90)), now() - make_interval(days => (n % 90))
FROM counts c CROSS JOIN LATERAL generate_series(1, greatest(0, 100 - c.current_count)) AS n;
